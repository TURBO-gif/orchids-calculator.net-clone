...
export default function IRACalculatorPage() {
  ...
  const calculate = () => {
    ...
    const taxDeferred = totalContributions * tax;
    const afterTaxValue = futureValue * (1 - tax);

    setResult({ futureValue, totalContributions, totalGrowth, taxDeferred, afterTaxValue, years });
  };

  const formatCurrency = (value: number) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(value);

  return (
    <CalculatorLayout title="IRA Calculator" description="Calculate your Traditional IRA growth with tax-deferred savings.">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="bg-card p-4 rounded-[3px] border border-border">
            <h3 className="font-bold text-base mb-4">IRA Details</h3>
            <div className="space-y-3">
              ...
              <div><label className="block text-sm font-bold mb-1">Expected Tax Rate in Retirement</label><div className="flex items-center gap-2"><input type="number" value={taxRate} onChange={(e) => setTaxRate(e.target.value)} className="flex-1 border border-input rounded-[3px] px-3 py-2 text-sm" /><span className="text-sm">%</span></div></div>
            </div>
          </div>
          ...
          {result ? (
            <>
              <div className="bg-card p-5 rounded-[3px] border-2 border-accent">
                <h3 className="font-bold text-lg mb-2 text-accent">Future Value (Pre-Tax)</h3>
              ...
              <div className="bg-card p-4 rounded-[3px] border border-border">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between py-1"><span className="text-muted-foreground">Tax Deferred Savings:</span><span className="font-semibold text-green-600">{formatCurrency(result.taxDeferred)}</span></div>
                  <div className="flex justify-between py-1 border-t border-border pt-2"><span className="font-bold">After-Tax Value:</span><span className="font-bold text-accent">{formatCurrency(result.afterTaxValue)}</span></div>
                </div>
              </div>
            </>
          ) : (
            <div className="bg-muted p-8 rounded-[3px] text-center text-muted-foreground"><p>Enter IRA details to calculate growth</p></div>
          )}
        </div>
      </div>
    </CalculatorLayout>
